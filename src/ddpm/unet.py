...
class SinusoidalPositionEmbeddings(nn.Module):  # noqa: D101
    ...
    def forward(self, time):  # noqa: D102
        return self.time_blocks(time)


class AttentionBlock(nn.Module):  # noqa: D101
...
class ResnetBlock(nn.Module):  # noqa: D101
    def __init__(self, *, in_channels, out_channels, dropout_rate=0.1, time_emb_dims=512, apply_attention=False):
        ...
        self.conv_1 = nn.Conv2d(
            in_channels=self.in_channels, out_channels=self.out_channels, kernel_size=3, stride=1, padding="same"
        )
        ...
        self.conv_2 = nn.Conv2d(
            in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=3, stride=1, padding="same"
        )
    ...
    def forward(self, x, t):  # noqa: D102
        # group 1
...
class DownSample(nn.Module):  # noqa: D101
    def __init__(self, channels):
        super().__init__()
        self.downsample = nn.Conv2d(in_channels=channels, out_channels=channels, kernel_size=3, stride=2, padding=1)

    def forward(self, x, *args):  # noqa: D102
        return self.downsample(x)


class UpSample(nn.Module):  # noqa: D101
...
class UNet(nn.Module):  # noqa: D101
    def __init__(
        self,
        input_channels=3,
        output_channels=3,
        ...
        self.first = nn.Conv2d(
            in_channels=input_channels, out_channels=base_channels, kernel_size=3, stride=1, padding="same"
        )
