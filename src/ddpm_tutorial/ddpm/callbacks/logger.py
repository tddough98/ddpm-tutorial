...
class LoggerCallback(Callback):  # noqa: D101
    ...
    def on_train_batch_end(
        self, trainer: "pl.Trainer", pl_module: "pl.LightningModule", outputs: dict, batch: Any, batch_idx: int
    ) -> None:
        if trainer.global_step % self.freq_train == 0:
            ...
            pl_module.log(
                "train/noise_loss", outputs["noise_loss"], on_step=True, on_epoch=False, prog_bar=True, logger=True
            )
            if outputs["vlb_loss"] is not None:
                pl_module.log(
                    "train/vlb_loss", outputs["vlb_loss"], on_step=True, on_epoch=False, prog_bar=True, logger=True
                )
    ...
    def on_validation_epoch_start(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        ...
        os.makedirs(f"gen_images/{pl_module.logger.version}", exist_ok=True)
        torchvision.utils.save_image(
            gen_images, f"gen_images/{pl_module.logger.version}/epoch={pl_module.current_epoch}.png"
        )  # Save the images
