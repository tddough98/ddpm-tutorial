...
def main():  # noqa: D103
    ...
    model = UNet(image_size=training_config.image_size, input_channels=training_config.image_channels).to(
    ...
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer=optimizer, T_max=len(train_dataloader) * training_config.num_epochs, last_epoch=-1, eta_min=1e-9
    )
    ...
    for epoch in range(training_config.start_epoch, training_config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            timesteps = torch.randint(
                0, diffusion_pipeline.num_timesteps, (batch_size,), device=training_config.device
            ).long()
