...
class TransformerPositionalEmbedding(nn.Module):
    ...
    def forward(self, timestep):  # noqa: D102
        # [bs, d_model]
        return self.pe_matrix[timestep].to(timestep.device)


class ConvBlock(nn.Module):  # noqa: D101
...
class DownsampleBlock(nn.Module):  # noqa: D101
    def __init__(self, in_channels, out_channels, stride, padding):
        super().__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, 3, stride=stride, padding=padding)

    def forward(self, input_tensor):  # noqa: D102
        x = self.conv(input_tensor)
        return x


class UpsampleBlock(nn.Module):  # noqa: D101
...
class ConvDownBlock(nn.Module):  # noqa: D101
    def __init__(self, in_channels, out_channels, num_layers, time_emb_channels, num_groups, downsample=True):
        ...
        self.downsample = (
            DownsampleBlock(in_channels=out_channels, out_channels=out_channels, stride=2, padding=1)
            if downsample
            else None
        )

    def forward(self, input_tensor, time_embedding):  # noqa: D102
        x = input_tensor
...
class ConvUpBlock(nn.Module):  # noqa: D101
    def __init__(self, in_channels, out_channels, num_layers, time_emb_channels, num_groups, upsample=True):
        ...
        self.upsample = UpsampleBlock(in_channels=out_channels, out_channels=out_channels) if upsample else None
...
class SelfAttentionBlock(nn.Module):
    ...
    def split_features_for_heads(self, tensor):  # noqa: D102
...
class AttentionDownBlock(nn.Module):  # noqa: D101
    def __init__(
        self, in_channels, out_channels, num_layers, time_emb_channels, num_groups, num_att_heads, downsample=True
    ):
        ...
        for i in range(num_layers):
            ...
            attention_block = SelfAttentionBlock(
                in_channels=out_channels, embedding_dim=out_channels, num_heads=num_att_heads, num_groups=num_groups
            )
    ...
    def forward(self, input_tensor, time_embedding):  # noqa: D102
        x = input_tensor
        for resnet_block, attention_block in zip(self.resnet_blocks, self.attention_blocks, strict=False):
            x = resnet_block(x, time_embedding)
            x = attention_block(x)
        if self.downsample:
            x = self.downsample(x)
...
class AttentionUpBlock(nn.Module):  # noqa: D101
    def __init__(
        self, in_channels, out_channels, num_layers, time_emb_channels, num_groups, num_att_heads, upsample=True
    ):
        """
        :param in_channels:
        :param out_channels:
...
class ResNetBlock(nn.Module):
    """In the original DDPM paper Wide ResNet was used (https://arxiv.org/pdf/1605.07146.pdf)."""

    def __init__(self, in_channels, out_channels, *, time_emb_channels=None, num_groups=8):
        super().__init__()
        self.time_embedding_projectile = (
            nn.Sequential(nn.SiLU(), nn.Linear(time_emb_channels, out_channels)) if time_emb_channels else None
        )
    ...
    def forward(self, x, time_embedding=None):  # noqa: D102
        input_tensor = x
        h = self.block1(x)
