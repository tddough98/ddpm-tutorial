...
from ddpm_tutorial.ddpm.model.distributions import mu_hat_xt_x0, mu_x_t, sigma_hat_xt_x0, sigma_x_t, x0_to_xt
from ddpm_tutorial.ddpm.variance_scheduler.abs_var_scheduler import Scheduler
...
class GaussianDDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        opt: type[torch.optim.Optimizer] | Callable[[Iterator[Parameter]], torch.optim.Optimizer],
        T: int,
    ...
    def training_step(self, batch: tuple[torch.Tensor, torch.Tensor], batch_idx: int):
        ...
        return {"loss": loss, "noise_loss": noise_loss, "vlb_loss": loss_vlb if use_vlb else None}

    def validation_step(self, batch: tuple[torch.Tensor, torch.Tensor], batch_idx: int):  # noqa: D102
        ...
        return {"loss": loss, "noise_loss": eps_loss, "vlb_loss": loss_vlb if self.vlb else None}

    def variational_loss(
        self, x_t: torch.Tensor, x_0: torch.Tensor, model_noise: torch.Tensor, v: torch.Tensor, t: torch.Tensor
    ):
        ...
        if torch.any(t_eq_0):
            p = torch.distributions.Normal(
                mu_x_t(x_t, t, model_noise, self.alphas_hat, self.betas, self.alphas),
    ...
    def generate(
        self, batch_size: int | None = None, T: int | None = None, get_intermediate_steps: bool = False
        ...
        X_noise = torch.randn(
            batch_size,
            self.input_channels,  # start with random noise sampled from N(0, 1)
            self.width,
            self.height,
            device=self.device,
        )
        ...
        for t in range(T - 1, -1, -1):
            ...
            X_noise = (
                1 / (torch.sqrt(alpha_t)) * (X_noise - ((1 - alpha_t) / torch.sqrt(1 - alpha_hat_t)) * eps) + sigma * z
            )  # denoise step from x_t to x_{t-1} following the DDPM paper. Differently from the
