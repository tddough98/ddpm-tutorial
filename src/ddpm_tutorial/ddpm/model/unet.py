...
def positional_embedding_vector(t: int, dim: int) -> torch.FloatTensor:
    """Create a sinusoidal positional embedding vector.
...
@torch.no_grad()
def init_zero(module: nn.Module) -> nn.Module:  # noqa: D103
    for p in module.parameters():
        torch.nn.init.zeros_(p)
    return module


class ResBlockTimeEmbed(nn.Module):  # noqa: D101
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        self.relu = nn.ReLU()
        self.l_embedding = nn.Sequential(nn.GELU(), nn.Linear(time_embed_size, out_channels))
    ...
    def find_max_num_groups(self, in_channels: int) -> int:  # noqa: D102
        for i in range(4, 0, -1):
            if in_channels % i == 0:
                return i
        raise Exception()

    def forward(self, x, time_embed):  # noqa: D102
        h = self.conv(x)
...
class ImageSelfAttention(nn.Module):  # noqa: D101
...
class UNetTimeStep(nn.Module):  # noqa: D101
    def __init__(
        self,
        channels: list[int],
        ...
        assert (
            ...
            f"{len(channels)} == {(len(kernel_sizes) + 1)} == "
        ...
        self.middle_block = ResBlockTimeEmbed(
            channels[-1], channels[-1], kernel_sizes[-1], strides[-1], paddings[-1], time_embed_size, p_dropouts[-1]
        )
        channels[0] *= 2  # because the output is the image plus the estimated variance coefficients
        self.upsample_blocks = nn.ModuleList(
            [
                ResBlockTimeEmbed(
                    (2 if i != 0 else 1) * channels[-i - 1],
                    channels[-i - 2],
                    kernel_sizes[-i - 1],
                    strides[-i - 1],
                    paddings[-i - 1],
